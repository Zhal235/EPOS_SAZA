<div x-data="{ activeTab: 'sales' }">
        ...
        <div class="bg-white rounded-xl shadow-lg border border-gray-200 p-2">
            <div class="flex gap-2">
                <button @click="activeTab = 'sales'" 
                        :class="activeTab === 'sales' ? 'bg-gradient-to-r from-indigo-600 to-purple-600 text-white shadow-md' : 'text-gray-600 hover:bg-gray-100'"
                ...
                <button @click="activeTab = 'analytics'" 
                        :class="activeTab === 'analytics' ? 'bg-gradient-to-r from-indigo-600 to-purple-600 text-white shadow-md' : 'text-gray-600 hover:bg-gray-100'"
                        class="flex-1 px-6 py-3 rounded-lg font-semibold transition-all duration-200 flex items-center justify-center gap-2">
    ...
    <!-- Filters Section -->
    <div class="bg-gradient-to-br from-gray-50 to-gray-100 rounded-xl shadow-lg border border-gray-300 p-6 mb-6" x-data="{ reportType: @entangle('reportType') }">
        ...
        <div class="flex flex-wrap gap-4 items-end">
            ...
            <div class="flex-1 min-w-[200px]">
                <label class="block text-sm font-bold text-gray-900 mb-2">
                    <i class="fas fa-chart-bar mr-1 text-indigo-600"></i>Report Type
                </label>
                <select wire:model.live="reportType" class="w-full rounded-lg border-gray-300 bg-white text-gray-900 font-medium focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500 shadow-sm px-4 py-2.5">
                    <option value="daily" class="text-gray-900 bg-white font-medium">Today</option>
                    <option value="weekly" class="text-gray-900 bg-white font-medium">This Week</option>
                    <option value="monthly" class="text-gray-900 bg-white font-medium">This Month</option>
                    <option value="custom" class="text-gray-900 bg-white font-medium">Custom Range</option>
                </select>
            </div>

            <!-- Date From - Only show when Custom Range -->
            <div class="flex-1 min-w-[200px]" x-show="reportType === 'custom'" x-transition>
                <label class="block text-sm font-bold text-gray-900 mb-2">
                    <i class="fas fa-calendar-alt mr-1 text-indigo-600"></i>From Date
                </label>
                <input type="date" wire:model="dateFrom" 
                       class="w-full rounded-lg border-gray-300 bg-white text-gray-900 font-medium focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500 shadow-sm px-4 py-2.5 [color-scheme:light]">
            </div>

            <!-- Date To - Only show when Custom Range -->
            <div class="flex-1 min-w-[200px]" x-show="reportType === 'custom'" x-transition>
                <label class="block text-sm font-bold text-gray-900 mb-2">
                    <i class="fas fa-calendar-check mr-1 text-indigo-600"></i>To Date
                </label>
            ...
            <div class="flex-1 min-w-[200px]">
                <label class="block text-sm font-bold text-gray-900 mb-2">
                    <i class="fas fa-user mr-1 text-indigo-600"></i>Cashier
                </label>
                <select wire:model="selectedCashier" class="w-full rounded-lg border-gray-300 bg-white text-gray-900 font-medium focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500 shadow-sm px-4 py-2.5">
                    <option value="" class="text-gray-900 bg-white font-medium">All Cashiers</option>
                    @foreach($cashiers as $cashier)
                        <option value="{{ $cashier->id }}" class="text-gray-900 bg-white font-medium">{{ $cashier->name }}</option>
                    @endforeach
                </select>
            </div>
            @endif
            ...
            <div class="flex-1 min-w-[200px]">
                ...
                <select wire:model="selectedPaymentMethod" class="w-full rounded-lg border-gray-300 bg-white text-gray-900 font-medium focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500 shadow-sm px-4 py-2.5">
                    <option value="" class="text-gray-900 bg-white font-medium">All Methods</option>
                    <option value="cash" class="text-gray-900 bg-white font-medium">Cash</option>
                    <option value="qris" class="text-gray-900 bg-white font-medium">QRIS</option>
                    <option value="rfid" class="text-gray-900 bg-white font-medium">RFID</option>
                    <option value="card" class="text-gray-900 bg-white font-medium">Card</option>
                    <option value="transfer" class="text-gray-900 bg-white font-medium">Transfer</option>
                </select>
            </div>
        </div>        <!-- Action Buttons -->
        <div class="flex flex-wrap gap-3 mt-6 pt-4 border-t border-gray-200">
            ...
            <button wire:click="generateReport" 
                    class="px-6 py-3 bg-white text-gray-700 border-2 border-gray-300 hover:border-indigo-500 rounded-lg font-medium transition-all">
    ...
    <div x-show="activeTab === 'sales'" x-transition>
        ...
        <!-- Total Sales -->
        <div class="bg-white rounded-xl shadow-lg border-l-4 border-blue-600 p-6 transform hover:scale-105 transition-transform duration-200">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-blue-600 text-sm font-semibold uppercase tracking-wide">Total Sales</p>
                    <h3 class="text-3xl font-bold text-gray-900 mt-2">Rp {{ number_format($totalSales, 0, ',', '.') }}</h3>
                    @if($totalSales > 0)
        ...
        <!-- Total Transactions -->
        <div class="bg-white rounded-xl shadow-lg border-l-4 border-green-600 p-6 transform hover:scale-105 transition-transform duration-200">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-green-600 text-sm font-semibold uppercase tracking-wide">Transactions</p>
        ...
        <!-- Average Transaction -->
        <div class="bg-white rounded-xl shadow-lg border-l-4 border-purple-600 p-6 transform hover:scale-105 transition-transform duration-200">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-purple-600 text-sm font-semibold uppercase tracking-wide">Average Sale</p>
                    <h3 class="text-3xl font-bold text-gray-900 mt-2">Rp {{ number_format($averageTransaction, 0, ',', '.') }}</h3>
        ...
        <!-- Total Profit -->
        <div class="bg-white rounded-xl shadow-lg border-l-4 border-orange-600 p-6 transform hover:scale-105 transition-transform duration-200">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-orange-600 text-sm font-semibold uppercase tracking-wide">Profit</p>
                    <h3 class="text-3xl font-bold text-gray-900 mt-2">Rp {{ number_format($totalProfit, 0, ',', '.') }}</h3>
                    @if($totalProfit > 0)
                        <p class="text-gray-600 text-xs mt-2">
        ...
        <div class="bg-white rounded-xl shadow-lg border border-gray-200 p-6">
                    ...
                    @foreach($topProducts as $index => $item)
                        <div class="flex items-center justify-between p-4 bg-gradient-to-r from-gray-50 to-gray-100 rounded-lg hover:shadow-md transition-shadow">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <p class="font-semibold text-gray-800">{{ $item->product->name ?? 'Unknown' }}</p>
            ...
            @else
                <div class="text-center py-12">
                    ...
                    <p class="text-gray-400 text-sm mt-1">Try adjusting your date range or filters</p>
                </div>
            @endif
        </div>
        ...
        <div class="bg-white rounded-xl shadow-lg border border-gray-200 p-6">
            ...
            @if(count($salesByPaymentMethod) > 0)
                <div class="space-y-2">
                    @foreach($salesByPaymentMethod as $payment)
                        ...
                        @endphp
                        <div class="p-4 bg-gradient-to-r from-gray-50 to-gray-100 rounded-lg hover:shadow-md transition-shadow">
                            <div class="flex items-center justify-between mb-2">
                                <div class="flex items-center gap-3">
                                    <div class="w-12 h-12 rounded-full flex items-center justify-center
                                        {{ $payment->payment_method === 'cash' ? 'bg-green-100' : '' }}
                                        {{ $payment->payment_method === 'qris' ? 'bg-blue-100' : '' }}
                                        {{ $payment->payment_method === 'rfid' ? 'bg-purple-100' : '' }}
                                        {{ !in_array($payment->payment_method, ['cash', 'qris', 'rfid']) ? 'bg-orange-100' : '' }}">
                                    ...
                                    <div>
                                        <p class="font-semibold text-gray-800 capitalize">{{ $payment->payment_method }}</p>
                                        <p class="text-sm text-gray-500">
            ...
            @else
                <div class="text-center py-12">
                    ...
                    <p class="text-gray-500 font-medium">No payment data available</p>
                    <p class="text-gray-400 text-sm mt-1">Transactions will appear here once available</p>
                </div>
            @endif
        </div>
    </div>
    ...
    @if($reportType === 'daily' && count($salesByHour) > 0)
        <div class="bg-white rounded-xl shadow-lg border border-gray-200 p-6 mb-6">
            <div class="flex items-center justify-between mb-4">
                ...
                <span class="text-sm text-gray-500 bg-indigo-50 px-3 py-1 rounded-full">
                    Hourly Breakdown
                </span>
            </div>
            <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8 gap-3">
                @foreach($salesByHour as $hourData)
                    ...
                    <div class="relative group">
                        <div class="p-4 bg-gradient-to-br from-indigo-50 to-purple-50 rounded-lg text-center hover:shadow-lg transition-all transform hover:-translate-y-1">
                            <p class="text-2xl font-bold text-indigo-600">{{ str_pad($hourData->hour, 2, '0', STR_PAD_LEFT) }}:00</p>
    ...
    @endif
        ...
        @if($totalTransactions === 0 && ($dateFrom || $dateTo))
            <div class="bg-white rounded-xl shadow-lg border-2 border-dashed border-gray-300 p-12 text-center">
                ...
                <h3 class="text-xl font-bold text-gray-800 mb-2">No Sales Data Found</h3>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div class="bg-white rounded-xl shadow-lg border border-gray-200 p-6">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <select wire:model.live="comparisonPeriod" class="text-sm rounded-lg border-gray-300 bg-white" style="color: #1f2937 !important;">
                    ...
                    <div class="bg-gradient-to-r from-green-50 to-emerald-50 rounded-lg p-4">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-2xl font-bold text-gray-900">Rp {{ number_format($growthData['current_sales'] ?? 0, 0, ',', '.') }}</p>
                            </div>
                            <div class="text-right">
                        ...
                        <div class="mt-2 pt-2 border-t border-green-200">
                            <p class="text-xs text-gray-500">{{ $growthData['compare_period_label'] ?? 'Previous' }}: Rp {{ number_format($growthData['compare_sales'] ?? 0, 0, ',', '.') }}</p>
                        </div>
                    </div>
                    ...
                    <div class="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg p-4">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-2xl font-bold text-gray-900">{{ number_format($growthData['current_transactions'] ?? 0) }}</p>
                            </div>
                            <div class="text-right">
                        ...
                        <div class="mt-2 pt-2 border-t border-blue-200">
                            <p class="text-xs text-gray-500">{{ $growthData['compare_period_label'] ?? 'Previous' }}: {{ number_format($growthData['compare_transactions'] ?? 0) }} transactions</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-lg border border-gray-200 p-6">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <span class="text-sm text-gray-500 bg-orange-50 px-3 py-1 rounded-full">
                        Top 5
                    </span>
                </div>
                        ...
                        @foreach($peakHours as $index => $peak)
                            <div class="flex items-center justify-between p-3 bg-gradient-to-r from-orange-50 to-amber-50 rounded-lg">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <p class="font-bold text-gray-900">{{ $peak['hour'] }}</p>
                                        <p class="text-xs text-gray-500">{{ $peak['count'] }} transactions</p>
                                    </div>
                                </div>
                                <div class="text-right">
                                    <p class="font-bold text-green-600">Rp {{ number_format($peak['sales'] / 1000, 0) }}K</p>
                                </div>
                            </div>
                        @endforeach
                    </div>
                @else
                    <div class="text-center py-8">
                        ...
                        <p class="text-gray-500">No peak hours data</p>
                    </div>
                @endif
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-lg border border-gray-200 p-6 mb-6">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-4">
                ...
                @foreach($salesTrend as $day)
                    ...
                    @endphp
                    <div class="relative group">
                        <div class="bg-gradient-to-br from-gray-50 to-gray-100 rounded-xl p-4 hover:shadow-xl transition-all">
                            <!-- Bar Chart -->
                            ...
                            <!-- Data Labels -->
                            <div class="text-center space-y-1 border-t border-gray-300 pt-2">
                                <p class="text-sm font-bold text-gray-900">{{ $day['label'] }}</p>
                                <p class="text-xs text-gray-600 font-medium">{{ $day['count'] }} sales</p>
                                <p class="text-xs font-bold bg-green-100 text-green-700 px-2 py-1 rounded">
        ...
        <div class="bg-white rounded-xl shadow-lg border border-gray-200 p-6 mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                @foreach($categoryPerformance as $category)
                    <div class="bg-gradient-to-br from-purple-50 to-pink-50 rounded-lg p-4 hover:shadow-md transition-shadow">
                        <div class="flex items-start justify-between mb-3">
                            <div>
                                <h4 class="font-bold text-gray-900">{{ $category['category'] }}</h4>
                                <p class="text-xs text-gray-500 mt-1">{{ $category['transaction_count'] }} transactions</p>
                            </div>
                        ...
                        <div class="space-y-2">
                            <div class="flex items-center justify-between">
                                ...
                                <span class="font-bold text-green-600">Rp {{ number_format($category['sales'], 0, ',', '.') }}</span>
                            </div>
                            <div class="flex items-center justify-between">
                                <span class="text-sm text-gray-600">Items Sold</span>
                                <span class="font-semibold text-gray-900">{{ $category['quantity'] }}</span>
                            </div>
                            <div class="flex items-center justify-between">
                                <span class="text-sm text-gray-600">Avg Value</span>
                                <span class="font-semibold text-indigo-600">Rp {{ number_format($category['avg_transaction_value'], 0, ',', '.') }}</span>
                            </div>
                        </div>
                    </div>
                @endforeach
                ...
                <table class="w-full border-collapse">
                    ...
                    <tbody>
                        @foreach($productPerformance as $index => $product)
                            <tr class="{{ $index % 2 == 0 ? 'bg-gray-700' : 'bg-gray-600' }} hover:bg-gray-500 transition-all duration-150">
                                <td class="px-6 py-4 border-b border-gray-200">
                                    <div class="font-bold text-gray-900 text-sm">{{ $product['product_name'] }}</div>
                                    <div class="text-xs text-gray-600 mt-1">
                                        <i class="fas fa-shopping-bag mr-1"></i>{{ $product['transaction_count'] }} transactions
                                    </div>
                                </td>
                                <td class="px-6 py-4 border-b border-gray-200">
                                    <span class="inline-flex items-center px-3 py-1 text-xs font-bold bg-purple-100 text-purple-900 rounded-full">
                                        {{ $product['category'] }}
                                    </span>
                                </td>
                                ...
                                <td class="px-6 py-4 text-right border-b border-gray-200">
                                    <span class="text-sm font-bold {{ $product['profit'] >= 0 ? 'text-blue-600' : 'text-red-600' }}">
                                        Rp {{ number_format($product['profit'], 0, ',', '.') }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 text-right border-b border-gray-200">
                                    <span class="inline-flex items-center px-3 py-1.5 text-xs font-bold rounded-lg
                                        {{ $product['profit_margin'] >= 30 ? 'bg-green-100 text-green-900' : '' }}
                                        {{ $product['profit_margin'] >= 15 && $product['profit_margin'] < 30 ? 'bg-yellow-100 text-yellow-900' : '' }}
                                        {{ $product['profit_margin'] >= 0 && $product['profit_margin'] < 15 ? 'bg-orange-100 text-orange-900' : '' }}
                                        {{ $product['profit_margin'] < 0 ? 'bg-red-100 text-red-900' : '' }}">
                                        {{ number_format($product['profit_margin'], 1) }}%
                                    </span>
                                </td>
                                <td class="px-6 py-4 text-right border-b border-gray-200">
                                    @if($product['current_stock'] == 0)
                                        <span class="inline-flex items-center px-3 py-1.5 text-xs font-bold bg-red-100 text-red-900 rounded-lg">
                                            <i class="fas fa-times-circle mr-1"></i> Out of Stock
                                        </span>
                                    @elseif($product['current_stock'] < 10)
                                        <span class="inline-flex items-center px-3 py-1.5 text-xs font-bold bg-yellow-100 text-yellow-900 rounded-lg">
                                            <i class="fas fa-exclamation-triangle mr-1"></i> {{ $product['current_stock'] }} (Low)
                                        </span>
                                    @else
                                        <span class="inline-flex items-center px-3 py-1.5 text-sm font-bold text-gray-900 bg-gray-200 rounded-lg">
                                            <i class="fas fa-check-circle text-green-600 mr-1"></i> {{ $product['current_stock'] }}
                                        </span>
                                    @endif
                                </td>
                            </tr>
